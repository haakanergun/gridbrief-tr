...
import { getExplorerResponse } from "../../../lib/epias/explorer-service";
import { parseExplorerRequest } from "../../../lib/epias/explorer-validation";
import {
  ...
  try {
    ...
    if (getEpiasConfigurationStatus() === "misconfigured") {
      throw new GatewayError(
        ...
        "The live EPİAŞ explorer gateway is not configured.",
        503,
      );
    }
    ...
    const explorerRequest = parseExplorerRequest(body);
    const response = await getExplorerResponse(explorerRequest);
